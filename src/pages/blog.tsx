...
import { fetchBlogPosts } from 'lib/dbConnect';
...
export default function EcommerceCheckout() {
  ...
  const category_id = router.query.category_id ? parseInt(router.query.category_id as string) : 1; // Set page to 1 if null
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const fetchedBlogPosts = await fetchBlogPosts(all);
  ...
  }, [page, firstTag, category_id]); // Fetch data whenever the page query changes
            ...
            <>
              <Collapse in={statestep !== 2}>
                <Grid container spacing={3}>
                  {blogPosts &&
                    blogPosts.result &&
                    blogPosts.result.map((post: BlogPost, index: number) => (
                      <Grid item md={4} xs={12} key={index}>
                        <NextLink href={`/blog/${post.slug}?id=${post.id}`} passHref>
                          <Link underline="none">
                            <Card>
                              <CardMedia
                                component="img"
